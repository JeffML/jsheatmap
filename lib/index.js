...
var Sterno = /** @class */ (function () {
    ...
    Sterno.prototype.getData = function (options) {
        ...
        var rows = this.rows.map(function (r) {
            var label = r[0], values = r[1], extra = r[2];
            high = Math.max.apply(Math, __spreadArrays(values, [high]));
            low = Math.min.apply(Math, __spreadArrays(values, [low]));
            if (low < 0)
                throw Error("negative input encountered");
            return { label: label, cells: { values: values, colors: [], scales: [], extra: extra } };
        });
